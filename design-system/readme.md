...
- **Tutor:** `<body data-theme="tutor">` — interfaz naranja con acentos lilas/violetas. El naranja de marca es **`#F6A56C`** y se pinta **lleno, con blanco encima** (decision de marca): nav, cabeceras y boton primario son ese color, no una version oscurecida de el. La combinacion da 2.0:1, asi que el texto sobre naranja va en cuerpo grande y peso semibold — y nunca en caption. La unica excepcion es `--color-primary-strong` (`#B85F2E`), reservado a **texto, iconos y enlaces sobre blanco**, donde el naranja de marca daria 2.0:1.
...
**Cámara.** `CameraCapture` es la cámara propia de la app, no el picker del sistema: existe porque la foto clínica necesita guía de encuadre y un paso de revisión antes de subir. Visor sobre `--surface-nav-deep` con `data-surface="dark"` — la misma superficie oscura que la navegación y el `Toast` — y controles flotando en los blancos translúcidos de la nav, sin paneles opacos sobre la imagen. El único color saturado es `--nav-accent`: modo activo, esquinas del encuadre, flash encendido y el botón "Usar foto"; el obturador queda blanco por convención. El modo `documento` dibuja **cuatro esquinas y nada más** (un rectángulo cerrado se leería como recorte ya aplicado) y `foto` no dibuja guía. Capturar siempre pasa por `revisando`: toma congelada, `Repetir` y `Usar foto` con el mismo peso, y el selector de modo desaparece. Sin permiso, el mismo criterio que `PermissionCard`: neutro y no rojo, con la consecuencia concreta y los ajustes como texto.
...
**Relleno solido.** El contenido sobre el relleno va siempre en `--color-primary-fill-fg`, nunca en blanco escrito a mano: en clinica el relleno es oscuro, en tutor el relleno ES el naranja de marca, y en los dos el contenido sale del mismo token. Analogo en la nav: `--text-on-nav` y `--nav-accent-fg` se invierten con el tema porque la nav del tutor es clara. Lo que tiene que ser oscuro por **funcion** y no por tema — el visor de `CameraCapture`, el `Toast` — usa la familia `--surface-immersive`, que no se invierte. `--color-primary-fill` (violeta en clinica y vet, naranja quemado en tutor) y `--color-primary-fill-hover` — boton primario, IconButton `solid`, dia elegido del calendario. `--color-primary` queda para bordes, tintes y superficies suaves.
...
`components/core/` — **Icon**, **Button**, **IconButton**, **Input**, **Textarea**, **Select**, **Checkbox**, **Radio**, **Switch**, **Card**, **Badge**, **Tag**, **Avatar**, **Tabs**, **Dialog**, **Toast**, **Tooltip**, **SearchField**, **Sheet**, **DatePicker**, **Calendar**, **DataTable**, **EmptyState**, **Skeleton**, **SkeletonText**, **InlineError**, **CameraCapture**, **FileDropzone**, **UploadItem**, **ProgressBar**, **PermissionCard**, **SocialButton**
